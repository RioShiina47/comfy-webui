...
def inject(assembler, chain_definition, chain_items):
    if not isinstance(chain_items, dict):
        print("[InfiniteTalk Injector] chain_items is not a dict, skipping.")
        return
    ...
    if num_chunks <= 1:
        print("[InfiniteTalk Injector] Only one chunk needed, no extension required.")
        return

    create_video_id = assembler.node_map.get(chain_definition['create_video_node'])
    if not create_video_id:
        print(f"[InfiniteTalk Injector] Error: Create Video node '{chain_definition['create_video_node']}' not found. Skipping.")
        return
    ...
    if not all([preprocessor_id, sampler_id, decoder_id]):
        print("[InfiniteTalk Injector] Error: Could not find preprocessor, sampler, or decoder nodes in the recipe map. Skipping.")
        return
    ...
    print(f"[InfiniteTalk Injector] Injected {num_extensions} extension chunks, for a total of {num_chunks} chunks.")