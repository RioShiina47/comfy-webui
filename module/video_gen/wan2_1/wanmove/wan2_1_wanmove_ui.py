        ...
        with gr.Accordion("Tracks Settings", open=False):
            ...
            for i in range(MAX_TRACK_SEGMENTS):
                with gr.Accordion(f"Track {i+1}", open=(i==0), visible=(i == 0)) as segment_group:
                    ...
                    bezier.change(fn=lambda x: gr.update(visible=x), inputs=[bezier], outputs=[bezier_controls_row], api_name=False)
...
def create_event_handlers(components: dict, all_components: dict, demo: gr.Blocks):
    ...
    add_btn.click(fn=add_segment, inputs=[count_state], outputs=add_outputs, api_name=False)
    del_btn.click(fn=delete_segment, inputs=[count_state], outputs=del_outputs, api_name=False)
