...
def register_shared_events(components, prefix, sdxl_gallery_height, demo):
    ...
    if all([aspect_ratio_dropdown, width_num, height_num]):
        def on_aspect_ratio_change(ratio_key, model_type):
            ...
            return w, h
        aspect_ratio_dropdown.change(fn=on_aspect_ratio_change, inputs=[aspect_ratio_dropdown, model_type_state], outputs=[width_num, height_num], api_name=False)
        ...
        if f"{prefix}_input_image" in components:
            ...
            image_input.upload(fn=on_image_upload, inputs=[image_input, model_type_state], outputs=[width_num, height_num, aspect_ratio_dropdown], api_name=False)
            image_input.change(fn=on_image_upload, inputs=[image_input, model_type_state], outputs=[width_num, height_num, aspect_ratio_dropdown], api_name=False)
