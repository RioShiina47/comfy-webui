def inject(assembler, chain_definition, chain_items):
    ...
    if ksampler_name not in assembler.node_map:
        print(f"Warning: KSampler node '{ksampler_name}' not found for SenseNova Reference chain. Skipping.")
        return
    ...
    if hasattr(assembler, '_get_node_template'):
    ...
    print(f"SenseNova Reference injector applied. Re-routed inputs through {min(len(chain_items), 10)} reference images.")
