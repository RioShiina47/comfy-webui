import gradio as gr
import os
from core.config import CIVITAI_API_KEY, EMBEDDING_DIR
    ...
    for i in range(len(embedding_sources)):
        ...
        if src == "File" and id_val:
            ...
            path, status_msg = resolve_and_download_asset(
                source=src,
                id_or_url=id_val,
                target_dir=EMBEDDING_DIR,
                api_key=CIVITAI_API_KEY,
                desc_prefix="Embedding"
            )
            if path is None:
...
def process_sensenova_reference_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    references = []
    ref_images = all_ui_values.get(key('sensenova_reference_images'), [])
    if not ref_images:
        return []
...
def process_joyai_image_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    references = []
    ref_images = all_ui_values.get(key('joyai_image_images')) or all_ui_values.get(key('joyai_reference_images'), [])
    if not ref_images:
        return []
...
process_joyai_reference_inputs = process_joyai_image_inputs
