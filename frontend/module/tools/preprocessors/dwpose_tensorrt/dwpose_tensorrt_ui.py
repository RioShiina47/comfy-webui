...
UI_INFO = {
    "workflow_recipe": "dwpose_tensorrt_recipe.yaml",
    "main_tab": "Tools",
...
def process_inputs(ui_values):
    ...
    if is_video:
        local_ui_values['input_video_filename'] = save_temp_file(input_file_obj, "dwpose_input", is_video=True)
        metadata = get_media_metadata(input_file_obj, is_video=True)
        local_ui_values['fps'] = metadata.get('fps', 30)
    else:
        local_ui_values['input_image_filename'] = save_temp_file(input_file_obj, "dwpose_input", is_video=False)
