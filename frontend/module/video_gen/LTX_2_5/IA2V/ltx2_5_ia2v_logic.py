...
from core.utils import save_temp_image, handle_seed, save_temp_audio
from core.input_processors import process_lora_inputs

WORKFLOW_RECIPE_PATH = "ltx2_5_ia2v_recipe.yaml"
WORKFLOW_RECIPE_2X = "ltx2_5_ia2v_2x_recipe.yaml"
WORKFLOW_RECIPE_3X = "ltx2_5_ia2v_3x_recipe.yaml"
...
def process_inputs(ui_values, seed_override=None):
    ...
    if start_image_pil is None:
        raise ValueError("Start image is required for Image & Audio to Video generation.")
    local_ui_values['start_image'] = save_temp_image(start_image_pil)
    
    audio_path = local_ui_values.get('audio_file')
    if audio_path is None:
        raise ValueError("Audio file is required for Image & Audio to Video generation.")
    ...
    try:
        metadata = get_media_metadata(audio_path, is_video=False)
    ...
    local_ui_values['video_length'] = int(round(duration_seconds * fps)) + 1
    ...
    local_ui_values['filename_prefix'] = f"video/{get_filename_prefix()}_ltx2.5_ia2v"
    local_ui_values['loras'] = process_lora_inputs(ui_values, 'ltx2_5_ia2v_lora')
