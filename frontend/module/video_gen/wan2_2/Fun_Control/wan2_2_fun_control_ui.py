import gradio as gr
import traceback
from .wan2_2_fun_control_logic import process_inputs, ASPECT_RATIO_PRESETS
...
def create_ui():
    ...
    with gr.Column():
        ...
        with gr.Row():
            components['ref_image'] = gr.Image(type="pil", label="Reference Image (for Style)", scale=1, height=282)
            components['control_video'] = gr.Video(label="Control Video (for Motion)", scale=1, height=282)
